...
const article string = `I am happy to join with you today in what will go down in history as the greatest demonstration for freedom in the history of our nation.
...
func main() {
	...
	slice := make([][]string, 0)
	for k,v := range *foo {
		slice = append(slice, []string{k, strconv.FormatInt(int64(v), 10)})
	}
	sort.Slice(slice, func(i, j int) bool {x,_ := strconv.ParseInt(slice[i][1], 10, 64); y,_ := strconv.ParseInt(slice[j][1], 10, 64) ; return  x>y })
	fmt.Println("TOP10:")
	for i:=0;i<10;i++ {
		fmt.Printf("%s: %s\n", slice[i][0], slice[i][1])
	}
