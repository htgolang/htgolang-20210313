...
type Persistent interface{
	Save([]module.User, string) error
	Load(string) ([]module.User, error)
}

type Factory struct{}

func (f Factory)CreatePersistent(ptype string) Persistent {
	var p Persistent
	switch ptype{
	case "csv":
		p = CsvPersistent{}
...
func (j JsonPersistent)Save(users []module.User, path string) error  {
...
func (j JsonPersistent)Load(path string) ([]module.User, error) {
	var users []module.User
	file,err := os.Open(path)
	if err != nil{
		return nil, err
