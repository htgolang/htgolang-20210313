...
func loadUserInfo() []module.User {
	...
	p = Factory{}.CreatePersistent(ptype)
	users, err := p.Load(path)
	if err != nil {
		log.Fatalf("load user info faild. %s", err)
	}
	return users
}
...
func SaveUserInfo() error {
	...
	err := CleanOldFile()
	if err != nil {
		return fmt.Errorf("save user info failed. %s", err)
	}

	p = Factory{}.CreatePersistent(ptype)
	err = p.Save(Users, path)
	if err != nil {
		return fmt.Errorf("save user info failed. %s", err)
	}
	return nil
...
func SelectPersistenType() {
	var Ptype string
	fmt.Print("请选择持久化方式(gob/csv/json): ")
	fmt.Scanln(&Ptype)
	switch Ptype {
	case "gob":
	...
	case "csv":
		...
		Confobj.SaveTo(ConfPath)
	case "json":
		Confobj.Section("DEFAULT").Key("persistent_type").SetValue("json")
		Confobj.SaveTo(ConfPath)
	default:
		fmt.Println("选择错误,请选择gob/csv/json")
	}
}
...
func CleanOldFile() error {
	...
	for _, f := range files {
		...
		} else if f.Name() == "user.json" {
			oldFilename = "user.json"
