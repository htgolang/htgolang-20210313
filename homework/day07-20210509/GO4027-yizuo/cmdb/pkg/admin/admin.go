...
// 用户登录日志记录,注意创建日志的目录必须存在
func UserLoginLogRecord(userName string) error {
	file, err := os.OpenFile(UserLoginLogFile, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		return err
	}
	defer file.Close()
	...
	// 填写日志内容，序列化成json
	var userLog map[string]string = map[string]string{
		"UserName":   userName,
		"CreateTime": time.Now().Format("2006-01-02 15:04:05"),
	}
	data, _ := json.Marshal(userLog)
