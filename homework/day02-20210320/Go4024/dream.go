...
func main() {
	var txt string
	txt = `I am happy to join with you today in what will go down in history as the greatest demonstration for freedom in the history of our nation.
...
// 	fmt.Println(txt)
	dream := make(map[string]int)

	for _, i := range txt {
		//fmt.Println(string(i))
		//只接受字母，string遍历后的值类型为int32
		if (i >= 65 && i <= 90) || (i >= 97 && i <= 122){
			//转成string格式作为map的key，与定义的类型匹配，或者定义dream的key类型为rune
			v,ok := dream[string(i)]
		if ok == true {
			dream[string(i)] = v +1
		} else {
