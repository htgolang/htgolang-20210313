...
func main() {
	...
	for _, s := range Dream {
		if unicode.IsLetter(s) {
			//if strconv.IsGraphic(s)
			Index, err := StringCount[string(s)]
			if err {
				StringCount[string(s)] = StringCount[string(s)] + 1
			} else {
				StringCount[string(s)] = Index + 1
				// fmt.Printf("抓到一个%c\n", s)
			}
		}

	}
	for zm, cs := range StringCount {
		fmt.Printf("字母%v 出现 %v\n", zm, cs)
	}
}
