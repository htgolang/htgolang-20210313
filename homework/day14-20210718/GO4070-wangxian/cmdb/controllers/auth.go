...
func (c *AuthenticationController) Prepare() {
	...
	if value := c.GetSession("uid"); value != nil {
		if uid, ok := value.(int64); ok {
			currentUser = services.UserService.QueryUserByID(uid)
		}
	}
	...
	//检查权限
	if !services.RoleService.IsPermission(currentUser.RoleId, c.Ctx.Input.URL()) {
		c.Ctx.WriteString("Permission denied")
		return
	}

	currentRoleModules := services.RoleService.GetModules(currentUser.RoleId)
	c.Data["CurrentUser"] = currentUser
	c.Data["CurrentRoleModules"] = currentRoleModules
