...
func (f *ModifyUserForm) Valid(v *validation.Validation) {
	f.Name = strings.TrimSpace(f.Name)
	f.Birthday = strings.TrimSpace(f.Birthday)
	f.Telephone = strings.TrimSpace(f.Telephone)
	f.Email = strings.TrimSpace(f.Email)
	...
	// 校验
	// ID是否存在
	modifyUser := services.UserService.GetById(f.Id)
	if modifyUser == nil {
		v.SetError("default", "用户信息不存在")
		return
	}
	...
	if result.Ok {
		if user := services.UserService.GetByName(f.Name); user != nil && user.Id != modifyUser.Id {
			v.SetError("name", "用户名已存在")
		}
	}
...
func (f *ModifyUserForm) FillModel(user *models.User) {
	f.Id = user.Id
	f.Name = user.Name
	// 注意user.Birthday
	f.Birthday = ""
