	...
	if c.Ctx.Input.IsPost() {
		...
		if err := c.ParseForm(form); err == nil {
			if ok, err := valid.Valid(form); err != nil {
				...
				// 存储数据
				services.UserService.Modify(form.User) // 根据存储结果给用户响应
		...
		if id, err := c.GetInt64("id"); err != nil {
			valid.SetError("default", "提交数据错误")
		} else if user := services.UserService.GetById(id); user == nil {
			valid.SetError("default", "提交ID错误")
		} else {
			form.FillModel(user)
		}
	}
